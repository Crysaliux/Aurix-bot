                ...
                if getlang is not None:
                    for language in Language.select().where(Language.guild_id == member.guild.id):
                        if language.lang == "ru":
                            ...
                            rem = discord.Embed(
                                title=f'На ваш сервер зашел рейдер {member}, айди {raiders.user_id}, класс {raiders.r_class}',
                                colour=0x39d0d6)
                            ...
                            channel = bot.get_channel(notification.channel_id)
                            rem = discord.Embed(
                                title=f'Your server has joined a raider {member}, id {raiders.user_id}, class {raiders.r_class}',
                                colour=0x39d0d6)
                            await channel.send(embed=rem)
                            for punishment in Punishment.select().where(Punishment.guild_id == member.guild.id):
                                if punishment.pun == 'ban':
        ...
        for language in Language.select().where(Language.guild_id == ctx.guild.id):
            if language.lang == "ru":
                ...
                help_emb.add_field(name='💵экономика',
                                   ...
                                   inline=False)
                help_emb.add_field(name='🔒антирейд', value='$notif_chan, $set_pun',
                                   inline=False)
                ...
                help_emb.add_field(name='💵economy',
                                   value='$user_create_account, $give_money, $balance, $user_balance, $set_money, $work, $item_create, $item_list, $buy_item, $item_delete, $account',
                                   inline=False)
                help_emb.add_field(name='🔒antiraid', value='$notif_chan, $set_pun',
                                   inline=False)
...
async def raider_add(ctx, id, r_class):
    if ctx.author.id != 830486806478848040:
        ...
        if getlang is not None:
            for language in Language.select().where(Language.guild_id == ctx.guild.id):
                if language.lang == "ru":
                    await ctx.send("Новый рейдер был успешно добавлен")

                else:
                    await ctx.send("Successfully added new raider")
...
async def notif_chan(ctx, id):
    ...
    if getlang is not None:
        for language in Language.select().where(Language.guild_id == ctx.guild.id):
            if language.lang == "ru":
                await ctx.send("Канал для уведомлений успешно установлен")

            else:
                await ctx.send("Successfully added channel for notifications")
...
async def set_pun(ctx, pun):
    ...
    if getlang is not None:
        for language in Language.select().where(Language.guild_id == ctx.guild.id):
            if language.lang == "ru":
                await ctx.send("Наказание для рейдеров успешно установлено")
